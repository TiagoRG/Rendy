...
namespace Rendy.Utilities
{
    public class Images
    {
        ...
        private static Bitmap CropToBanner(Image image)
        {
            ...
            var heigthRatio = (float)originalHeight / destinationSize.Height;

            var ratio = Math.Min(widthRatio, heigthRatio);

            var widthScale = Convert.ToInt32(destinationSize.Width * ratio);
            var heigthScale = Convert.ToInt32(destinationSize.Height * ratio);

            var startX = (originalWidth - widthScale) / 2;
            var startY = (originalHeight - heigthScale) / 2;

            var sourceRectangle = new Rectangle(startX, startY, widthScale, heigthScale);
        ...
        private Image DrawTextToImage(Image image, string header, string subheader)
        {
            ...
            var brushGrey = new SolidBrush(ColorTranslator.FromHtml("#E3E3E3"));
        ...
        private async Task<Image> FetchImageAsync(string url)
        {
            ...
            {
                var backupResponse = await client.GetAsync("https://images.unsplash.com/photo-1447752875215-b2761acb3c5d?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1050&q=80");
